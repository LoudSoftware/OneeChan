...
export class PresenceUpdateListener extends Listener {    
    ...
    public async run(_oldPresence: Presence, newPresence: Presence) {
        if (!newPresence.member?.voice.channel || newPresence.guild === null) return;

        const channel = newPresence.member.voice.channel;
        const settings = getSettings(newPresence.guild.id);
        if (!settings || channel.parentId !== settings.categoryChannelID) return;

        const activities = channel.members.map((member: GuildMember) => member.presence?.activities.flatMap((activity) => activity.name)).flat() || [];
        ...
        if (activities.length > 0) {
            ...
            if (mostCommonActivity && channel.name !== mostCommonActivity) {
                this.container.logger.info(`Setting channel name to: ${mostCommonActivity}`);
